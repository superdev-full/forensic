...
for I_vowel = 1:num_vowels
    load(['.\results\GMM', num2str(mix(I_vowel), '%02.0f'), '_' vowel_labels{I_vowel}, '.mat'], 'log_scores_train_LogReg_ss', 'log_scores_train_LogReg_ds', 'log_scores', 'Indices_comparisons');
    log_scores_cell{I_vowel} = log_scores(:, iter(I_vowel)); 
    II_pairs_cell{I_vowel} = Indices_comparisons; % assume order of comparisons is same for all phonemes 
